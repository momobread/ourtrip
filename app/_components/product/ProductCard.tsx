...
const ProductCard = ({ data, category }: { data: ProductType; category: string }) => {
  ...
  return (
    <li className="relative flex w-[30rem] flex-col items-center justify-center rounded-xl border border-grey-400 sm:h-[45rem]">
      <Link href={`/${category}/${product_num}`}>
        <p className="absolute right-[1rem] top-[1rem] z-30 flex">
          <span className="font-bold text-myred-300">{product_liked}</span>
          <svg
        ...
        <div className="relative h-[35rem] w-[30rem] rounded-t-xl">
          <Image src={product_img} className="rounded-t-xl" alt="hotelimage" fill />
        </div>
